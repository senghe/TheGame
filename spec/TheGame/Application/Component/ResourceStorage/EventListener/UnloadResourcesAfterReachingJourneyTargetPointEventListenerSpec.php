...
final class UnloadResourcesAfterReachingJourneyTargetPointEventListenerSpec extends ObjectBehavior
{
    ...
    public function it_dispatches_resources_from_fleet_transporting_and_reaching_target_point_to_the_storages(
        NavigatorInterface $navigator,
        CommandBusInterface $commandBus,
        ...
        $event = new FleetHasReachedJourneyTargetPointEvent(
            MissionType::Transport->value, $fleetId, $targetCoordinates, $resourcesLoad
        );
        $this->__invoke($event);
    }

    public function it_dispatches_resources_from_fleet_stationing_and_reaching_target_point_to_the_storages(
        NavigatorInterface $navigator,
        CommandBusInterface $commandBus,
        PlanetIdInterface $planetId,
    ): void {
        ...
        $event = new FleetHasReachedJourneyTargetPointEvent(
            MissionType::Stationing->value, $fleetId, $targetCoordinates, $resourcesLoad
        );
        $this->__invoke($event);
    }

    public function it_does_nothing_when_mission_is_attack(): void
    {
        $targetCoordinates = "[4:5:6]";
        $fleetId = "086a3343-22cb-4673-88c8-368e9e9709f3";
        $resourcesLoad = [
        ...
        $event = new FleetHasReachedJourneyTargetPointEvent(
            MissionType::Attack->value, $fleetId, $targetCoordinates, $resourcesLoad
        );
        $this->__invoke($event);
    }
